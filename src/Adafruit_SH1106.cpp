...
// dim = true: display is dimmed
// dim = false: display is normal
void Adafruit_SH1106::brightness(uint8_t brightness) {

    if (_vccstate == SH1106_EXTERNALVCC) {
      brightness = min(brightness, 0x9F);
    } else {
      brightness = min(brightness, 0xCF);
    }
  
  // the range of contrast to too small to be really useful
  // it is useful to dim the display
  SH1106_command(SH1106_SETCONTRAST);
  SH1106_command(brightness);
}
...
void Adafruit_SH1106::display(void) {
    ...
    //NeoSerial.println(TWBR, DEC);
    //NeoSerial.println(TWSR & 0x3, DEC);
